...
def test_growth_rendering()->None:
 with tempfile.TemporaryDirectory() as t:
  ...
  timeline={"nodes":[{"id":"NODE-0001","title":"首次返还","chapters":{"start":1,"end":3},"status":"verified"},{"id":"NODE-0002","title":"宗门成长","chapters":{"start":45,"end":52},"status":"partial"},{"id":"NODE-0066","title":"仙塔突破","chapters":{"start":500,"end":508},"status":"partial"}]}
  record={"id":"CHAR-0001","name":"徐霄","status":"alive","role":"protagonist","documents":{"profile":"docs/02-characters/徐霄.md"},"first_appearance":{"chapter":1,"title":"万倍返还系统","node":"NODE-0001"},"cultivation":{"current":"炼气境"},"cultivation_change":"大乘三重","affiliations":["缥缈宗"],"source_chapters":[1,2,45,50,500,508],"related_nodes":["NODE-0001","NODE-0002","NODE-0066"]}
  write(generated/"timeline.yaml",timeline)
  write(generated/"characters.yaml",{"updated_at":"2026-07-27","characters":[record]})
  ...
  first_event=growth.first(record,{item["id"]:item for item in timeline["nodes"]})
  assert first_event is not None
  assert first_event.start==1 and first_event.end==1
  assert first_event.title=="首次登场：万倍返还系统"
  assert "独立历史快照" in first_event.ability
  assert "大乘三重" not in first_event.ability
  assert "大乘三重" not in first_event.impact
